...
const showSuccessToast = (): void => {
  notify.success(__("Success toast preview"), {
    description: __(
...
const showErrorToast = (): void => {
  notify.error(__("Error toast preview"), {
    description: __(
  ...
  columnHelper.accessor("name", {
    ...
    cell: ({ row }) =>
      h("div", { class: "min-w-44 space-y-1" }, [
        h("p", { class: "font-medium text-foreground" }, __(row.original.name)),
        h(
          "p",
  ...
  <AppLayout :breadcrumbs="dashboardBreadcrumbs">
    <div class="flex flex-col gap-5 p-4 md:gap-6">
      ...
      <section class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <Card class="border-sidebar-border/70 bg-background/80 shadow-none">
          <CardHeader class="space-y-2">
            <CardTitle class="text-base">{{ __("Example form") }}</CardTitle>
            <p class="text-sm leading-6 text-muted-foreground">
              {{
                __(
          ...
          <CardContent class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="grid gap-2">
              ...
              <Input id="demo_owner" v-model="formOwner" class="rounded-xl" />
            </div>
            <div class="grid gap-2 sm:col-span-2">
              <Label for="demo_release_state">{{ __("Release state") }}</Label>
              <Select v-model="formReleaseState">
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Beta">{{ __("Beta") }}</SelectItem>
                  <SelectItem value="Preview">{{ __("Preview") }}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div class="flex flex-wrap gap-3 sm:col-span-2">
              <Button class="rounded-xl" @click="activeDialog = 'create'">
                {{ __("Create example") }}
              </Button>
              <Button
                variant="outline"
        ...
        <Card class="border-sidebar-border/70 bg-background/80 shadow-none">
          <CardHeader class="space-y-2">
            <CardTitle class="text-base">{{ __("Badge set") }}</CardTitle>
            <p class="text-sm leading-6 text-muted-foreground">
              {{
                __(
      ...
      <section class="space-y-3">
        ...
        <DataTable
          ...
          :route="demoDashboard"
          :only="['examplesTable']"
          :empty-title="__('No sandbox examples match the current filters.')"
          :empty-description="
            __(
      ...
      <SheetContent side="right" class="sm:max-w-md">
        ...
        <div class="space-y-4 px-6 py-6">
          ...
          <div class="space-y-2">
            ...
            <Input
              id="sheet_focus"
              :model-value="__('Filter controls and preview states')"
              class="rounded-xl"
              readonly
            />
          </div>
