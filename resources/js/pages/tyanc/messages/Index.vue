...
async function openNewConversationDialog() {
    if (
        ...
        try {
            ...
            toast.error(__('Unable to load the messaging workspace.'));
            return;
        }
    }
...
function mergeWorkspace(payload: MessagesPageProps) {
    ...
    createConversationAbility.value =
        payload.abilities?.createConversation ??
        createConversationAbility.value;
    archiveConversationAbility.value =
        payload.abilities?.archiveConversation ??
        archiveConversationAbility.value;
    deleteConversationAbility.value =
        payload.abilities?.deleteConversation ??
        deleteConversationAbility.value;
...
async function loadConversation(
    ...
    try {
        ...
        if (!silent) {
            toast.error(__('Unable to load the selected conversation.'));
        }
    } finally {
        if (silent) {
...
async function toggleViewMode() {
    ...
    try {
        ...
        toast.error(__('Unable to switch conversation view.'));
...
async function archiveConversation(conversationId: string) {
    ...
    try {
        ...
        toast.success(
            archiving
                ? __('Conversation archived.')
                : __('Conversation unarchived.'),
        );
    } catch {
        toast.error(__('Unable to archive this conversation.'));
    }
}
...
async function deleteConversation(conversationId: string) {
    try {
        ...
        toast.success(__('Conversation deleted.'));
    } catch {
        toast.error(__('Unable to delete this conversation.'));
    }
}
        ...
        >
            ...
            <main class="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
                ...
                <MessageThread
                    ...
                    @typing="handleTyping"
                    @archive="
                        () => void archiveConversation(selectedConversation!.id)
                    "
                    @delete-conversation="
