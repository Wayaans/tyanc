...
import { AlertTriangle, ArrowLeft, Clock, ExternalLink } from "lucide-vue-next";
...
function fromDraftValues(
  ...
  return {
    ...
    username:
      typeof v.username === "string" ? v.username : (fallback.username ?? ""),
    ...
    timezone: typeof v.timezone === "string" ? v.timezone : fallback.timezone,
    roles: Array.isArray(v.roles) ? (v.roles as string[]) : [...fallback.roles],
...
function doDelete(note: string) {
  ...
  router.delete(destroy.url({ user: props.user.id }), {
    ...
    onError: (responseErrors) => {
      deleteErrors.value = responseErrors as Partial<Record<string, string>>;
      ...
      >
        <form class="space-y-6 p-6 md:p-8" @submit.prevent="handleSubmit">
          ...
          <div class="flex items-center justify-end gap-3">
            ...
            <Button type="submit" :disabled="processing || isFormLocked">
        ...
        <div class="p-6 md:p-8">
          ...
          <div class="mt-4 flex flex-wrap items-center gap-3">
            ...
            <Button
              ...
              @click="handleDelete"
            >
              <AlertTriangle v-if="confirmingDelete" class="size-4" />
