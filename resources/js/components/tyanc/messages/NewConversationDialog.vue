    ...
    if (!response.ok) {
      if (response.status === 422 && payload?.errors) {
        ...
        errors.value = {
          message:
            payload?.message ??
            __("Unable to start the conversation. Please try again."),
        };
      }
...
<template>
  <Dialog :open="props.open" @update:open="(v) => emit('update:open', v)">
    <DialogScrollContent class="flex max-h-[90dvh] flex-col sm:max-w-lg">
      ...
      <div class="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto py-1 pr-1">
        <!-- Contact search + list -->
        <div class="flex flex-col gap-2">
          ...
          >
            ...
            <p class="text-sm font-medium text-foreground">
              {{ __("You do not have permission to start a conversation.") }}
            </p>
          ...
          >
            <li v-for="contact in filteredContacts" :key="contact.id">
              <div
                ...
                @keydown.enter.prevent="toggleContact(contact.id)"
                @keydown.space.prevent="toggleContact(contact.id)"
              >
                <Checkbox
                  ...
                  @click.stop
                  @update:checked="
                    (checked) => setContactSelection(contact.id, checked)
                  "
                />
      ...
      <DialogFooter class="mt-2 gap-2 border-t border-border pt-4 sm:gap-2">
        <Button
          variant="outline"
          :disabled="processing"
